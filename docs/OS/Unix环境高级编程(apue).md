...
- 做流控，给每个客户端限速，在指定时间间隔里，对每个客户端发放cps个token（每个token可指定流量大小），并且规划好单位时间内，共放发brust个token（防止单一客户端累积大量token，然后在某个时间内全部用完，导致其他客户端无流量可用）
- > [通过信号的实现](https://github.com/zyxstar/exam_c/tree/master/apue/learn/signal/mytbf)
