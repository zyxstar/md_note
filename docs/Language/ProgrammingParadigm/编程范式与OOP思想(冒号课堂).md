...
### 稳定依赖原则
- 依赖反转原则 __可重述__ 为 __稳定依赖原则__(Stable Dependencies Principle, SDP)，模块应朝着稳定的方向依赖
...
### DI vs DIP
- 侧重点不同：DI强调依赖的来源－完全由外部提供，该依赖自然最好是抽象的，但并非首要要求； DIP则强调依赖的抽象类
...
控制反转更侧重保证组件的可重用性，依赖反转更侧重保证组件的可维护性
