...
![img](../../imgs/ren_domain_social.png)
...
========
## 响应队列
使用消息机制，在controller的`after_filter publish_event`上设置需要侦听的action，然后可以新增N个响应订阅者，对此action进行处理


### 全局提醒
适用于任务、@我、私信、留言、挑战、物流等场景

- 新增响应订阅者：`提醒产生subscriber`，负责感兴趣的行为的监听，收到消息后，写入`用户提醒表`(用户，提醒类型，[提醒html模板编号]，提醒对应动作action编号，完成状态)
- 新增响应订阅者：`提醒完成subscriber`，
    - 负责监听 `提醒对应动作action` 的完成，更新`用户提醒表`，认为提醒完成
    - 负责监听 `提醒超时间action` 的完成，类似对于每日任务未完成的，也更新`用户提醒表`，第二天无需显示

### 好友动态
适用于好友相关动态(关注、开通应用、领取任务、发表话题)

- 新增响应订阅者：`好友动态subscriber`，负责感兴趣的行为的监听，收到消息后，写入`好友动态表`（用户，时间，action编号，生成url）
...
依据惠爱提供的相关资料，我们设计了初步一个营养方案生成的概念模型，图中两个几个颜色的区别：红色部分是 __过程化数据__ ，用于记录具有时间维度的数据，会周期性的更新，它的更新将导致模型重新计算；蓝色部分是 __配置规则__ 的，用于描述数据或规则的，它的数据的增加或减少将影响模型的准确性；绿色是 __具体的事或物__，它是基础数据，需要不断完善的，它的多少将影响模型的可用性

![img](../../imgs/ren_domain_huiai.png)

相关的说明如下：

1. 顾客根据问卷 __周期性__ 的来完善相关的 __顾客状态__、__顾客病史__、__顾客体质__、__顾客饮食习惯__
    - 当顾客填写“其它”时，需要后台人员来对该描述进行评估，以确定是否要完善问卷，及相应的食材逻辑关系
2. 文档中提及的逻辑关系图中被定义为 __食材逻辑关系__，它根据食材属性和顾客的相关情况以及节气等情况来对食材进行匹配，它有两种类型：__适宜关系__ 和 __不宜关系__，前者用于增加搜索结果，后者用于过虑搜索的结果。这些关系是可增加和删除的
3. 根据 __食材逻辑关系__ 即可推导出待选食材，后者对应系统即可生成 __待选成品库__
4. 根据顾客对食材分类、食材、成品的喜恶程度，对应系统中存在的食材与成品表，推导出 __成品白名单__ 及 __成品黑名单__
    - 系统需建立该反馈渠道，顾客饮食习惯包含一部分，但不完全
5. 使用 __餐单生成规则__, 对 __待选成品库__ 的产品，根据顾客喜恶的名单，将其进行筛选与排序，再对应 __餐单模板__ 与 __成品类别__ 进行一天的营养方案的生成。如果需生成多天的方案，需增加 __成品重复频率__ 的限制，以防止成品重复率过高
6. 根据 __方案生成规则__，多次调用餐单生成，形成顾客的一定周期内的 __营养方案__
7. 营养师可根据顾客相关情况，对 __营养方案__ 进行调整
...
- 意志力是一种生理机能，是一种有限资源
- 意志力会消耗能量，糖，低血糖意志力就比较弱，难集中注意力，不能控制自己负面情绪
- 做选择会消耗意志力，买车时会有很多升级配置，聪明的销售让你刚来的时候先对一些花钱少的配置进行选择，等连续决策到选累时，再介绍价格贵或者根本没用的选项
- 冒险也需要意志力，意志力弱时，法官会做出风险低的决策
- 广告用美女，能降低男人的意志力；名车对女性的略有影响，美男则没的影响
- 在房间里放一面镜子就能让自己自控力增加不少
- 意志力是一种通用资源，有效办法是“常立志”，做自己不习惯做的事情，比如有意识的用左手，强迫不说脏话
- 中国的意识力，君子慎独


- 要知道你的不利条件，在某些情况下可能是你的有利条件，而巨人的所谓有利条件，在某些情况下可能是他的不利条件
- 绝对不能按照对手的打法去跟他玩，有时候使用非常规手段
- 维护现有的社会格局和强调遵守游戏规则，那是高富帅的事，而改变规则则是屌丝的特权
- 法国打越南，越南游击队一直赢，一直到1951年越南决定使用正规军，使用法国的打法跟法国打，立即遭遇惨败
